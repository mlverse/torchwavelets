...
*Note: This is the beginning of a package for doing wavelet analysis in R, based on `torch`. As of this writing, it comprises the code (slightly modified) used in the CRC publication, Deep Learning and Scientific Computing with R `torch`.*

*Wavelet analysis is a vast domain. Contributors are more than welcome!*

## A real-world example: Chaffinch's song

This example is discussed in Arnt Vistnes' fabulous book, *Physics of Oscillations and Waves*. We download the audio file from his website:
...
We use `torchaudio` to load the file, and convert from stereo to mono using `tuneR`.
...
![Chaffinch's song.](vignettes/resources/wav-chaffinch.png){#fig-wav-chaffinch fig-alt="A sequence of several bursts, varying in height and thickness."}
...
![Chaffinch's song, Fourier spectrum (excerpt).](vignettes/resources/wav-chaffinch-fft.png){#fig-wav-chaffinch-fft fig-alt="A sequence of spikes, skewed to the left."}
...
For better comparability with wavelet diagrams, we plot frequencies on a log scale.
...
![Chaffinch's song, spectrogram.](vignettes/resources/wav-chaffinch-spectrogram.png){#fig-wav-chaffinch-spectrogram fig-alt="A two-dimensional heat map, indicating high values, strongly varying in time, around log frequency 3.5 Hz."}

The spectrogram already shows a distinctive pattern. Let's see what can be done with wavelet analysis. Like Vistnes, we use the *Morlet* wavelet. Having experimented with a few different scales, I agree with Vistnes that `s = 48` makes for an excellent choice:
...
![Chaffinch's song, wavelet diagram.](vignettes/resources/wav-chaffinch-waveletdiag.png){#fig-wav-chaffinch-waveletdiag fig-alt="A two-dimensional heat map, of high resolution, that strongly emphasizes how patterns vary in time."}

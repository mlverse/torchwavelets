...
`wavelet_transform()`, in its constructor, requires just a single argument to be passed: the length of the series. However, while the other parameters do have defaults, it is important to know how they influence the result.
For the wavelet used (parameter `wavelet`) this is self-evident.
`dt` indicates the time between samples, expressed in the unit we want to work with. For example, with the Niño data, if we want to report results in years, we pass in a `dt` of ~1/12.
For `dj`, the matter is more complex. Below, we discuss how `wavelet_transform()` computes the set of scale parameters used; `dj` determines the spacing between them. The smaller `dj`, the closer together the scales used. What makes for a useful setting depends on the width (in Fourier space) of the wavelet function in question. Citing Torrence and Compo:
...
Here $s_0$ is the smallest *resolvable* scale. This is done by `wavelet_transform()` internally, the strategy being derived from the Nykvist theorem: To be resolvable, $s_0$ needs to be chosen so that the equivalent Fourier period amounts to approximately $2 \delta_t$.

## Cone of influence

Since the Fourier Transform assumes the data to be periodic, errors will occur at the start and the end of the series. The *cone of influence* (COI) is defined making use of the construct of $e$-folding time. In general,$e$-folding time is defined as the time it takes for a quantity to decrease to $\frac{1}{e}$ of its previous value.
Here, $e$-folding time is computed, _for each scale_, for the autocorrelation of wavelet power. Concretely, it is chosen so that wavelet power for a discontinuity at an edge drops by a factor of $\frac{1}{e^2}$. This ensures that edge effects are negligible beyond this point.
...
As Torrence & Combo explain, $\tau$ is also useful in evaluating the ontological status of of a peak in the scaleogram:
